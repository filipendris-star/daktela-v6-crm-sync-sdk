...
final class DaktelaAdapterTest extends TestCase
{
    ...
    #[\PHPUnit\Framework\Attributes\DataProvider('unusableResponseProvider')]
    public function testAnUnusableResponseFailsTheEntityEnumerationInsteadOfEndingIt(int $status, string $body): void
    {
        $adapter = $this->adapterWithHttpResponse($status, $body, __FUNCTION__ . $status);

        $this->expectException(AdapterException::class);
        iterator_to_array($adapter->iterateEntity('contact'));
    }
    ...
    public function testAGenuinelyEmptyPageStillEndsTheEnumerationCleanly(): void
    {
        ...
        self::assertSame([], iterator_to_array($adapter->iterateEntity('contact')));
    }
     ...
     */
    private function adapterWithHttpResponse(int $status, string $body, string $seed): DaktelaAdapter
    {
        ...
        $http->method('send')->willReturnCallback(
            fn () => new \GuzzleHttp\Psr7\Response($status, [], $body),
        );
